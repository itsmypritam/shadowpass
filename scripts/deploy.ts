/**
 * Deploy the ShadowPass contract to a Midnight network (undeployed by default; use --network preview|preprod for public networks).
 *
...
// ShadowPass contract has no witnesses, so its private state is empty ({}).
...
async function main() {
  console.log('\n╔══════════════════════════════════════════════════════════════╗');
  console.log(`║  Deploy ShadowPass to ${network}`);
  console.log('╚══════════════════════════════════════════════════════════════╝\n');
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      // initialPrivateState (empty here — the ShadowPass contract has no
      // witnesses). args is the contract constructor's arguments: empty for
      // the no-arg constructor. (Statically-typed contracts can omit
