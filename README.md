...
verifiable contract address. There is no backend on the live site — the whole
flow runs in your browser:

1. **Connect Lace** (Midnight wallet) on the `preprod` network.
2. Pick the result you want recorded and enter your **private** score.
3. The circuit proves the comparison in your browser (keys served over HTTPS,
   proofs generated by the wallet-configured proof server).
4. Lace signs and submits. Only the boolean is written to the ledger.

Install [Lace](https://lace.io), create a Midnight wallet on Preprod, and grab
test funds from the [Midnight faucet](https://faucet.testnet.midnight.network/).
Your browser needs a **DUST** balance to balance the transaction — convert some
tNIGHT to DUST inside Lace (receive → dust generator).
